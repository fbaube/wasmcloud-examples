...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost { errResponseJSON(w,
	...
	if err := json.Unmarshal(b, &req); err != nil {
		errResponseJSON(w, http.StatusBadRequest,
			fmt.Sprintf("error with json input: %s", err.Error()))
		return
	}
	...
	// If we can't write at this point there 
	// isn't much we can do so eat the error
...
// Instead, we call func `handleRequest` when an HTTP request is received.
func main() {}
